...
def simulate_F(n):
    scales = np.exp(np.random.randn(n) * np.sqrt(0.5))
    return norm.cdf(x[None, :], loc=0.0, scale=scales[:, None])
...
fig1.write_image("Bands_Gaussian_Scale.png", scale=2.0)
...
fig2.write_image("Width_Gaussian_Scale.png", scale=2.0)
